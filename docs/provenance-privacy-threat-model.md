...
| Prompt, location, thumbnail, or private assertion leaks | The profile has no prompt/location inputs, disables thumbnails, emits no task body, repo URL, PR URL, changed files, or arbitrary receipt fields. | Future assertion additions require renewed privacy review. |
...
| Malicious manifest exhausts parser resources | Asset, sidecar, and report sizes are bounded; exceptions become concise non-traceback failures. | Native parser CPU complexity is not fully controlled by byte limits. Run untrusted bulk inputs in an OS sandbox. |
